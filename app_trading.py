...
bt, df_viewer = st.tabs(["Backtesting", "View Data"])
...
class TradingStrategy:
    ...
    def fetch_data(self):
        if self.data_source == "API":
            ...
            self.close_data = vbt.YFData.download(
                self.symbol, start=start_date_tz, end=end_date_tz
            ).get("Close")
            self.open_data = vbt.YFData.download(
                self.symbol, start=start_date_tz, end=end_date_tz
            ).get("Open")
            self.high_data = vbt.YFData.download(
                self.symbol, start=start_date_tz, end=end_date_tz
            ).get("High")
            self.low_data = vbt.YFData.download(
                self.symbol, start=start_date_tz, end=end_date_tz
            ).get("Low")
            ...
            self.open_data = df["Open"]
            self.high_data = df["High"]
            self.low_data = df["Low"]

    def calculate_signals(self):
        if self.strategy_name == "MA Crossover":
            short_ema = vbt.MA.run(
                self.close_data,
            ...
            long_ema = vbt.MA.run(
                self.close_data, self.params["long_period"], short_name="slow", ewm=True
            )
            ...
            bb = vbt.BBANDS.run(
                self.close_data, self.params["bb_period"], self.params["bb_std"]
            )
...
with st.sidebar:
    ...
    with strategies_container:
        num_strategies = st.number_input(
            "Number of Strategies", min_value=1, max_value=10, value=1
        )

        for i in range(num_strategies):
            st.subheader(f"Strategy {i + 1}")
            strategy = st.selectbox(
                f"Select Strategy {i + 1}",
                ["MA Crossover", "RSI Strategy", "Bollinger Bands", "MACD"],
                key=f"strategy_{i}",
            )
            ...
            if strategy == "MA Crossover":
                params["short_period"] = st.number_input(
                    f"Short MA Period {i + 1}",
                    value=10,
                    min_value=1,
                    key=f"short_ma_{i}",
                )
                params["long_period"] = st.number_input(
                    f"Long MA Period {i + 1}", value=20, min_value=1, key=f"long_ma_{i}"
                )
            elif strategy == "RSI Strategy":
                params["rsi_period"] = st.number_input(
                    f"RSI Period {i + 1}", value=14, min_value=1, key=f"rsi_period_{i}"
                )
                params["rsi_oversold"] = st.number_input(
                    f"RSI Oversold Level {i + 1}",
                    value=30,
                    min_value=1,
                    max_value=50,
                    key=f"rsi_oversold_{i}",
                )
                params["rsi_overbought"] = st.number_input(
                    f"RSI Overbought Level {i + 1}",
                    value=70,
                    min_value=50,
                    max_value=100,
                    key=f"rsi_overbought_{i}",
                )
            elif strategy == "Bollinger Bands":
                params["bb_period"] = st.number_input(
                    f"BB Period {i + 1}", value=20, min_value=1, key=f"bb_period_{i}"
                )
                params["bb_std"] = st.number_input(
                    f"BB Standard Deviations {i + 1}",
                    value=2.0,
                    min_value=0.1,
                    key=f"bb_std_{i}",
                )
                ...
                params["macd_fast"] = st.number_input(
                    f"MACD Fast Period {i + 1}",
                    value=12,
                    min_value=1,
                    key=f"macd_fast_{i}",
                )
                params["macd_slow"] = st.number_input(
                    f"MACD Slow Period {i + 1}",
                    value=26,
                    min_value=1,
                    key=f"macd_slow_{i}",
                )
                params["macd_signal"] = st.number_input(
                    f"MACD Signal Period {i + 1}",
                    value=9,
                    min_value=1,
    ...
    size_type = st.selectbox(
        "Size Type", ["amount", "value", "percent"], index=2
...
if backtest_clicked:
    ...
    for strategy, params in strategy_params:
        trading_strategy = TradingStrategy(
            symbol, start_date, end_date, strategy, params, data_source, csv_file
        )
        ...
        if combined_entries is None:
            ...
            if combine_method == "AND":
                ...
                combined_entries = combined_entries | trading_strategy.entries
                combined_exits = combined_entries | trading_strategy.exits
    ...
    portfolio = final_strategy.run_backtest(
        initial_equity, size, size_type, fees, direction
    )
    ...
    tab1, tab2, tab3 = st.tabs(
        ["Backtesting Stats", "List of Trades", "Portfolio Plot"]
    )
...
def generate_excel_file(strategies):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        for strat in st.session_state.strategies:
            ...
            trades_df["Entry Timestamp"] = trades_df["Entry Timestamp"].apply(
                lambda a: pd.to_datetime(a).date()
            )
            trades_df["Exit Timestamp"] = trades_df["Exit Timestamp"].apply(
                lambda a: pd.to_datetime(a).date()
            )
...
def generate_html():
    ...
    for strat in st.session_state.strategies:
        ...
        html_content += (
            f'<h2>Portfolio Plot</h2><img src="data:image/png;base64,{img_str}">'
        )
...
with df_viewer:
    if data_source == "API":
        if symbol and start_date and end_date:
            ...
            df = vbt.YFData.download(symbol, start=start_date_tz, end=end_date_tz).get()
